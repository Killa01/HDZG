...
import wyf.ytl.Research;

import CityInfo.FarmingSkill;
import CityInfo.FishingSkill;
import CityInfo.GameView;
import Layer.CityDrawable;
import Layer.Skill;
import Layer.SuiXinBuSkill;
import MeetableLayer.MiningSkill;
...
public class Hero implements Externalizable{
	...
	public int strength = 100;//英雄体力，
	public int maxStrength = 100;//体力上限，会随着等级增加
	public int level = 1;//英雄等级
	...
	public int armyWithMe = 6000;//跟随自己的军队
	...
	public int title = 0;//官衔 
	int warTank = 0;//随身携带的战车
	int warTower = 0;//随身携带的箭垛
	int strengthGrowSpan = 1;
	public int basicAttack = 18;//基础攻击力
	public int basicDefend = 18;//基础防御力


	ArrayList<Bitmap []> animationSegment = new ArrayList<Bitmap []>();//存放英雄所有的动画段，每个动画段为一个一维数组
	public ArrayList<CityDrawable> cityList = new ArrayList<CityDrawable>();//存放英雄拥有的城池
	public ArrayList<General> generalList = new ArrayList<General>();//存放【跟随】英雄的将领
	public ArrayList<Research> researchList = new ArrayList<Research>();//存放现有的科研项目
	public HashMap<Integer,Skill> heroSkill = new HashMap<Integer,Skill>();//存放英雄的所有技能
	
	public HeroThread ht;//负责英雄动画换帧的线程
	public HeroGoThread hgt;//负责英雄走路的线程
	public HeroBackDataThread hbdt;//负责后台数据更改的线程，如粮草减少，科研进度/敌人来袭等
	//构造器：
	...
	//技能类型，包括0:生活技能、1:战斗技能、2:普通技能
		...
		this.heroSkill.put(fishingSkill. fishingSkill, fishingSkill);
		MiningSkill miningSkill = new MiningSkill(MINING, "采矿", 500, 0, this);
		this.heroSkill.put(miningSkill.miningSkill, miningSkill);
		FarmingSkill farmSkill = new FarmingSkill(FARMING, "农耕", 500, 0, this);
		this.heroSkill.put(farmSkill. farmSkill, farmSkill);	
